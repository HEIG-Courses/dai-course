...
# List available distributions, then install the LATEST Ubuntu version
# shown in YOUR list (do not install the unversioned "Ubuntu" entry)
wsl --list --online
...
Your Ubuntu home directory (`~`) is where **all your projects go**. It is
reachable from Windows at `\\wsl.localhost\Ubuntu-<version>\home\<user>`:
...
```sh
# Installs the Apple command-line tools, which include Git
...
```sh
# Find the latest LTS Temurin version (21.x.y-tem at the time of writing)
sdk list java
...
java --version    # prints Temurin 21 (or newer LTS)
...
# Windows 10 (instead of the above)
...
Java 21 installation (on Windows: from WSL), and its terminal opens your
Unix shell.
...
| SDKMAN! | `sdk version` |
| Java LTS | `java --version` shows Temurin 21+ |
| Maven | `mvn --version` |
